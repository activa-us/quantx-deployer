#!/usr/bin/env python3
"""
Strategy name : 3003EURUSD15MMO
Source        : MQLGen EURUSD M15 660212443.mq5
Symbol        : EURUSD
CLIENT_ID     : 3003
...
Logic:
  ...
  Long  when SMA(7) crosses above SMA(8), SMA(11) is below SMA(42),
        and CCI(44) is rising.
  Short when SMA(7) crosses below SMA(8), SMA(11) is above SMA(42),
        and CCI(44) is falling.
  Exit long  after two consecutive bearish 15-minute candles with body >= 8 pips.
  Exit short after two consecutive bullish 15-minute candles with body >= 8 pips.
...
Production standards checklist:
  ...
  - position state persisted under state/positions_EURUSD.json
...
CLIENT_ID = int(os.getenv("IBKR_CLIENT_ID", "3003"))
...
FAST_CROSS_MA_PERIOD = 7
SLOW_CROSS_MA_PERIOD = 8
FAST_FILTER_MA_PERIOD = 11
SLOW_FILTER_MA_PERIOD = 42
CCI_PERIOD = 44
CANDLE_BODY_PIPS = 8
...
def cci(high: pd.Series, low: pd.Series, close: pd.Series, period: int) -> pd.Series:
    ...
    typical_sma = typical_price.rolling(period).mean()
    mean_deviation = typical_price.rolling(period).apply(
        lambda values: abs(values - values.mean()).mean(),
        raw=False,
    )
    return (typical_price - typical_sma) / (0.015 * mean_deviation.replace(0, math.nan))


def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["ma_fast_cross"] = sma(df["close"], FAST_CROSS_MA_PERIOD)
    df["ma_slow_cross"] = sma(df["close"], SLOW_CROSS_MA_PERIOD)
    df["ma_fast_filter"] = sma(df["close"], FAST_FILTER_MA_PERIOD)
    df["ma_slow_filter"] = sma(df["close"], SLOW_FILTER_MA_PERIOD)
    df["cci"] = cci(
        df["high"],
        df["low"],
        df["close"],
...
def get_entry_signal(df: pd.DataFrame) -> str:
    ...
    ma_cross_long = (
        prev["ma_fast_cross"] > prev["ma_slow_cross"] + SIGMA
        and older["ma_fast_cross"] < older["ma_slow_cross"] - SIGMA
    )
    ma_cross_short = (
        prev["ma_fast_cross"] < prev["ma_slow_cross"] - SIGMA
        and older["ma_fast_cross"] > older["ma_slow_cross"] + SIGMA
    ...
    ma_filter_long = prev["ma_fast_filter"] < prev["ma_slow_filter"] - SIGMA
    ma_filter_short = prev["ma_fast_filter"] > prev["ma_slow_filter"] + SIGMA

    cci_long = prev["cci"] > older["cci"] + SIGMA
    cci_short = prev["cci"] < older["cci"] - SIGMA

    can_long = ma_cross_long and ma_filter_long and cci_long
    can_short = ma_cross_short and ma_filter_short and cci_short
...
def get_exit_signal(df: pd.DataFrame, current_position: float) -> bool:
    ...
    body_threshold = PIP_SIZE * CANDLE_BODY_PIPS
    bearish = (lookback["open"] - lookback["close"]) >= body_threshold
    bullish = (lookback["close"] - lookback["open"]) >= body_threshold
...
def process_completed_bar(ib: IB, contract: Contract, df: pd.DataFrame) -> None:
    ...
    logger.info(
        (
            ...
            "MA7=%.6f MA8=%.6f MA11=%.6f MA42=%.6f CCI44=%.4f pos=%.4f"
        ),
        bar_time,
        completed_bar["open"],
        completed_bar["close"],
        completed_bar["ma_fast_cross"],
        completed_bar["ma_slow_cross"],
        completed_bar["ma_fast_filter"],
        completed_bar["ma_slow_filter"],
        completed_bar["cci"],
        current_position,
    )

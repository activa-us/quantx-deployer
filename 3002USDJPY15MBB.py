#!/usr/bin/env python3
"""
Strategy name : 3002USDJPY15MBB
Source        : MQLGen USDJPY M15 1462666988.mq5
Symbol        : USDJPY
CLIENT_ID     : 3002
...
Logic:
  ...
  Long  when the current bar opens back above the prior lower Bollinger Band,
        DI+ is below DI-, and Stochastic %K crosses below %D.
  Short when the current bar opens back below the prior upper Bollinger Band,
        DI+ is above DI-, and Stochastic %K crosses above %D.
  Exit long  when the current bar opens below the prior lower exit band.
  Exit short when the current bar opens above the prior upper exit band.
...
Production standards checklist:
  ...
  - position state persisted under state/positions_USDJPY.json
...
import pandas as pd
import pytz
from ib_insync import Forex, IB, Contract, MarketOrder, util


STRATEGY_NAME = "3002USDJPY15MBB"
SYMBOL = "USDJPY"
SEC_TYPE = "CASH"
EXCHANGE = "IDEALPRO"
CURRENCY = "JPY"
...
CLIENT_ID = int(os.getenv("IBKR_CLIENT_ID", "3002"))
...
# MQL inputs.
ENTRY_LOTS = float(os.getenv("ENTRY_LOTS", "0.10"))
FX_UNITS_PER_LOT = float(os.getenv("FX_UNITS_PER_LOT", "100000"))
ENTRY_AMOUNT = float(os.getenv("ENTRY_AMOUNT", str(ENTRY_LOTS * FX_UNITS_PER_LOT)))
ENTRY_BB_PERIOD = 21
ENTRY_BB_DEVIATION = 1.77
ADX_PERIOD = 48
STO_K_PERIOD = 4
STO_D_PERIOD = 2
STO_SLOWING = 3
EXIT_BB_PERIOD = 48
EXIT_BB_DEVIATION = 3.28
SIGMA = 0.000001
...
def bollinger_bands(close: pd.Series, period: int, deviation: float) -> tuple[pd.Series, pd.Series]:
...
def directional_indicators(
    ...
    plus_di = 100.0 * plus_dm_smoothed / atr.replace(0, math.nan)
    minus_di = 100.0 * minus_dm_smoothed / atr.replace(0, math.nan)
...
def stochastic_signal(
    ...
    raw_k = 100.0 * (close - lowest_low) / (highest_high - lowest_low).replace(0, math.nan)
    main_k = raw_k.rolling(slowing).mean()
    signal_d = main_k.rolling(d_period).mean()
    return main_k, signal_d


def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["entry_bb_upper"], df["entry_bb_lower"] = bollinger_bands(
        df["close"],
        ENTRY_BB_PERIOD,
        ENTRY_BB_DEVIATION,
    )
    df["plus_di"], df["minus_di"] = directional_indicators(
        df["high"],
        df["low"],
        df["close"],
        ADX_PERIOD,
    )
    df["stoch_k"], df["stoch_d"] = stochastic_signal(
        df["high"],
        df["low"],
        df["close"],
        STO_K_PERIOD,
    ...
    df["exit_bb_upper"], df["exit_bb_lower"] = bollinger_bands(
        df["close"],
        EXIT_BB_PERIOD,
        EXIT_BB_DEVIATION,
    )
...
def get_entry_signal(df: pd.DataFrame) -> str:
    ...
    bb_long = curr_open > prev["entry_bb_lower"] + SIGMA and prev["open"] < prev["entry_bb_lower"] - SIGMA
    bb_short = curr_open < prev["entry_bb_upper"] - SIGMA and prev["open"] > prev["entry_bb_upper"] + SIGMA

    di_long = prev["plus_di"] < prev["minus_di"] - SIGMA
    di_short = prev["plus_di"] > prev["minus_di"] + SIGMA

    older = df.iloc[-3]
    stoch_long = older["stoch_k"] > older["stoch_d"] + SIGMA and prev["stoch_k"] < prev["stoch_d"] - SIGMA
    stoch_short = older["stoch_k"] < older["stoch_d"] - SIGMA and prev["stoch_k"] > prev["stoch_d"] + SIGMA

    can_long = bb_long and di_long and stoch_long
    can_short = bb_short and di_short and stoch_short
...
def get_exit_signal(df: pd.DataFrame, current_position: float) -> bool:
    prev = df.iloc[-2]
    curr_open = df.iloc[-1]["open"]
    long_exit = curr_open < prev["exit_bb_lower"] - SIGMA and prev["open"] > prev["exit_bb_lower"] + SIGMA
    short_exit = curr_open > prev["exit_bb_upper"] + SIGMA and prev["open"] < prev["exit_bb_upper"] - SIGMA
...
def process_completed_bar(ib: IB, contract: Contract, df: pd.DataFrame) -> None:
    ...
    logger.info(
        (
            "Bar closed | %s open=%.6f close=%.6f stochK=%.4f stochD=%.4f "
            "DI+=%.4f DI-=%.4f entryBB=(%.6f, %.6f) exitBB=(%.6f, %.6f) pos=%.4f"
        ),
        bar_time,
        completed_bar["open"],
        completed_bar["close"],
        completed_bar["stoch_k"],
        completed_bar["stoch_d"],
        completed_bar["plus_di"],
        completed_bar["minus_di"],
        completed_bar["entry_bb_lower"],
        completed_bar["entry_bb_upper"],
        completed_bar["exit_bb_lower"],
        completed_bar["exit_bb_upper"],
        current_position,
    )
